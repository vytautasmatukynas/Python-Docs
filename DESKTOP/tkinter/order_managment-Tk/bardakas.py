...
def query_database():
    conn = psycopg2.connect(
        ...
        database='d6m17s2cvstg9p',
        user='swxagcutfydrgj',
        password='f332093a6ac38b8accf1f68812368ca18b8dfde3b221156d2ee539a0d3543679',
        port='5432'
    )
...
my_tree.column("PROJEKTAS", anchor=CENTER, width=100, stretch=NO)
...
def update_record_double(e):
    ...
    pv_box['values'] = ('EQ', 'METACO', 'AUTOSABINA', 'SRS', 'KAVERA', 'KAGNETA', 'MITRONAS', 'MJ',
                        'KASTAGA', 'METGA', 'HITECH', 'PLASTME', 'ULMAS', 'TRUKME', 'MILTECHA')
    ...
    km1_box = ttk.Combobox(data_frame, font='sans 10', state='normal', textvariable=StringVar(), justify='center')
...
def komponentu_atsargos():
    ...
    k_atsargos_frame.focus_force()

    conn = psycopg2.connect(
        host='ec2-34-247-151-118.eu-west-1.compute.amazonaws.com',
        database='d1r3um0hjj3p34',
        user='itbtjgiitmvmns',
        password='a83c74b5d445e37a171e6ea4bfcb872c81a3977c443bb8d5b7b95bc00b50afab',
        port='5432'
    )
...
def stelazas():
    ...
    stelazas_frame.focus_force()

    conn = psycopg2.connect(
        host='ec2-63-32-7-190.eu-west-1.compute.amazonaws.com',
        database='df5vftjc7kv1h9',
        user='chxonafwtutypl',
        password='4425e4b16f67df00746f7bcddd21c5ee7670c23fa71a6c305f11bc9282147781',
        port='5432'
    )
    ...
    def query_database():
        ...
        c.execute("""SELECT * FROM stelazas""")
        records = c.fetchall()
    ...
    def new_record():
        ...
        vt_box = ttk.Combobox(data_frame, font='sans 10', state='normal', textvariable=StringVar(), width=59)
        vt_box['values'] = ('A1', 'A2', 'A3', 'A4', 'A5', 'A6', 'C1', 'C2', 'C3', 'C4')
        vt_box.grid(row=1, column=1, sticky="nsew")
...
# Copy csv
def save():
    query = """SELECT imone, konstruktorius, projektas, terminas, statusas FROM uzsakymai"""
